...
def train(net, trainloader, optimizer, criterion, device, ema_net=None):
    rot_criterion = nn.CrossEntropyLoss()  # for 4-way rotation classification
    ...
    for batch_idx, (data, feature, label) in enumerate(trainloader):
        ...
        cls_logits, rot_logits = net(data, feature, return_aux=True)
        cls_loss = cal_loss(cls_logits, label)
        rot_loss = rot_criterion(rot_logits, rot_labels)
        loss = cls_loss + 0.3 * rot_loss                    # total_loss
