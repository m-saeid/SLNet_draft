...
def apply_random_rotation(pc):
    """Rotate point cloud pc [B, N, 3] by a random multiple of 90° about Z and return labels."""
    k = torch.randint(0, 4, (pc.size(0),), dtype=torch.long)
    angles = (k.float() * (np.pi / 2)).to(pc.device)
    cos = torch.cos(angles).unsqueeze(-1).unsqueeze(-1)  # [B,1,1]
    sin = torch.sin(angles).unsqueeze(-1).unsqueeze(-1)  # [B,1,1]
    zeros = torch.zeros_like(cos)
    ones = torch.ones_like(cos)
    R = torch.cat([
        torch.cat([cos, -sin, zeros], dim=2),
        torch.cat([sin,  cos, zeros], dim=2),
        torch.cat([zeros, zeros, ones], dim=2),
    ], dim=1)  # [B,3,3]
    pc_rot = torch.einsum('bnc, bcd -> bnd', pc, R)
    return pc_rot, k
...
class Classification(nn.Module):
    def __init__(self,
                 n=1024,
                 embed=[3, 32, 'mlp', 'no', 0.4, 1000.0, 100.0],
                 res_dim_ratio=1.0,
                 bias=False,
                 use_xyz=True,
        ...
        if rotation_head:
            self.aux_rotation_head = nn.Sequential(
                nn.Linear(last_dim, 256),
                nn.ReLU(),
                nn.Linear(256, 4)  # 4 rotation classes: 0°, 90°, 180°, 270°
            )

    def forward(self, xyz, feature=None): #, return_aux=False):
        xyz_list, f_list = self.encoder(xyz=xyz, x=None, feature=feature)
        x = F.adaptive_max_pool1d(f_list[-1], 1).squeeze(dim=-1)
        # return self.classifier(x)
        class_logits = self.classifier(x)
        if self.rotation_head: #return_aux:
            rot_logits = self.aux_rotation_head(x)
...
def parse_args():
    ...
    parser.add_argument('--classifier_mode', type=str, default='mlp_medium', help='classifier_mode')
    ...
    parser.add_argument('--learning_rate_sgd', default=0.1, type=float, help='learning rate in training adamW')
    parser.add_argument('--weight_decay_sgd', type=float, default=2e-4, help='weight decay adamW')
    parser.add_argument('--learning_rate_adamw', default=1e-3, type=float, help='learning rate in training adamW')
    parser.add_argument('--weight_decay_adamw', type=float, default=1e-2, help='weight decay adamW')

    parser.add_argument('--scheduler', type=str, default='CosineAnnealingLR', help='[CosineAnnealingLR, CosineAnnealing_Warmup]')
    parser.add_argument('--criterion', type=str, default='cal_loss', help='[cal_loss, CrossEntropy_Smoothing]')
    ...
    parser.add_argument('--rotation_head', default='yes', type=str, help='[no, yes]')
    return parser.parse_args()

def main():
    args = parse_args()

    args.model = 'cls_modelnet' #norm_mode
    ...
    net = Classification(n=args.n,
                         ...
                         rotation_head=True if args.rotation_head=='yes' else False)
    
    if args.criterion=="CrossEntropy_Smoothing":
        criterion = nn.CrossEntropyLoss(label_smoothing=0.1)  # smooth 10% to uniform :contentReference[oaicite:9]{index=9} # cal_loss
    elif args.criterion=="cal_loss":
        criterion = cal_loss
    ...
    if args.optimizer=="adamw":
        # Recommended initial LR for AdamW: 1e-3 or 5e-4
        optimizer = torch.optim.AdamW(net.parameters(), lr=args.learning_rate_adamw, weight_decay=args.weight_decay_adamw)  # lr=1e-3, weight_decay=1e-2
        # Lower than SGD’s 1e-1, per best practices :contentReference[oaicite:4]{index=4}
    ...
    if args.scheduler=='CosineAnnealingLR':
    ...
    elif args.scheduler=='CosineAnnealing_Warmup':
        ...
        scheduler = get_cosine_with_hard_restarts_schedule_with_warmup(
            ...
            num_cycles=0.5
        )  # warmup from 0→lr, then cosine decay :contentReference[oaicite:6]{index=6}

    for epoch in range(start_epoch, args.epoch):
        printf('Epoch(%d/%s) Learning Rate %s:' % (epoch + 1, args.epoch, optimizer.param_groups[0]['lr']))
        rotation_head = True if args.rotation_head=='yes' else False
        train_out = train(net, train_loader, optimizer, criterion, device, ema_net, rotation_head)
    ...
    results = {
        ...
        "learning_rate_sgd": args.learning_rate_sgd,
        "weight_decay_sgd": args.weight_decay_sgd,
        "learning_rate_adamw": args.learning_rate_adamw,
...
def train(net, trainloader, optimizer, criterion, device, ema_net=None, rotation_head=False):
    rot_criterion = nn.CrossEntropyLoss() if rotation_head else None # for 4-way rotation classification
    ...
    for batch_idx, (data, feature, label) in enumerate(trainloader):
        ...
        # Apply random Z-rotation and get rotation labels
        if rotation_head:
            data_rot, rot_labels = apply_random_rotation(data.permute(0,2,1))  # [B,N,3]→rotation
            data_rot = data_rot.permute(0,2,1)                                # [B,3,N]
            rot_labels = rot_labels.to(device)   
            cls_logits, rot_logits = net(data_rot, feature)
            cls_loss = criterion(cls_logits, label)
            rot_loss = rot_criterion(rot_logits, rot_labels)
            loss = cls_loss + 0.3 * rot_loss  # λ=0.3
        else:
            cls_logits = net(data, feature)
            loss = criterion(cls_logits, label)
        ...
        if rotation_head:
            ...
            rot_acc = (rot_preds == rot_labels).float().mean().item()
        else:
            rot_acc = 0

        train_loss += loss.item()
        # preds = cls_loss.max(dim=1)[1]
        preds = cls_logits.argmax(dim=1) if not rotation_head else rot_logits.argmax(dim=1)
        ...
        #             'Loss: %.3f | Acc: %.3f%% (%d/%d)'
        #             % (train_loss / (batch_idx + 1), 100. * correct / total, correct, total), f" rot_acc: {round(rot_acc,2)}")
    
        progress_bar(
                    batch_idx, len(trainloader),
                    'Loss: %.3f | Acc: %.3f%% (%d/%d) | rot_acc: %.2f'
                    % (train_loss / (batch_idx + 1), 100. * correct / total, correct, total, rot_acc)
                )
...
if 0:#__name__ == '__main__':
    ...
    feature = None # torch.rand(2, 16, 1024) if embd[-1] == 'pointhop' else None
