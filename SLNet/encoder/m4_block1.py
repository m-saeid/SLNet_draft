...
class Block1(nn.Module):    # reduce d  and  residual
    def __init__(self, ch, out_ch, transfer_mode='global', block1_mode='cross_residual',  blocks=1,
                 adaptive_ratio=0.9, adaptive_res_dim_ratio=0.25, agmentive_res_dim_ratio=4, bias=True, use_xyz=True):
        ...
        if transfer_mode == 'no':
            ...
            self.embd = MEMBlock(in_ch, out_ch, mode['num_groups'], mode['hidden_ratio'], mode['rnn_type'], mode['use_spatial_descriptor'], mode['use_residual'])
        ...
        for i in range(blocks):
            ...
            if block1_mode=='cross_residual_expert':
                operation.append(CrossResidualExpert(in_ch, out_ch, adaptive_ratio=adaptive_ratio,
                                                bottleneck_ratio=adaptive_res_dim_ratio,
                                                augment_expand_ratio=agmentive_res_dim_ratio,
                                                bias=bias))
            elif block1_mode=='cross_residual':
                operation.append(CrossResidual(in_ch, out_ch, adaptive_ratio=adaptive_ratio,
