...
class Encoder(nn.Module):
    def __init__(self,
                 ...
                 num_blocks1=[1, 1, 2, 1],
                 transfer_mode = ['none', 'none', 'none', 'none'],
                 block1_mode = ['cross_residual_expert', 'cross_residual_expert', 'cross_residual_expert', 'cross_residual_expert'],
                 adaptive_ratio_1=[0.9, 0.9, 0.9, 0.9],
                 adaptive_res_dim_ratio_1=[0.25, 0.25, 0.25, 0.25],
                 agmentive_res_dim_ratio_1=[4, 4, 4, 4],

                 num_blocks2=[1, 1, 2, 1],
                 block2_mode = ['cross_residual_expert', 'cross_residual_expert', 'cross_residual_expert', 'cross_residual_expert'],
                 adaptive_ratio_2=[1,1,1,1],  # [0.9, 0.9, 0.9, 0.9],
                 adaptive_res_dim_ratio_2=[0.25, 0.25, 0.25, 0.25],
                 agmentive_res_dim_ratio_2=[2, 2, 2, 2],
        ...
        for i in range(self.stages):
            ...
            self.blocks1_list.append(Block1(ch=last_ch, out_ch=out_ch, transfer_mode=transfer_mode[i],
                                            block1_mode=block1_mode[i], blocks=num_blocks1[i],
                                            adaptive_ratio=adaptive_ratio_1[i],
                                            adaptive_res_dim_ratio=adaptive_res_dim_ratio_1[i],
                                            agmentive_res_dim_ratio=agmentive_res_dim_ratio_1[i],
                                            bias=True, use_xyz=True))
            
            self.agg_list.append(Agg("adaptive_max_pool1d"))

            self.blocks2_list.append(Block2(channels=out_ch, block1_mode=block2_mode[i], blocks=num_blocks2[i],
                                            adaptive_ratio=adaptive_ratio_2[i],
                                            adaptive_res_dim_ratio=adaptive_res_dim_ratio_2[i],
                                            agmentive_res_dim_ratio=agmentive_res_dim_ratio_2[i],
                                            bias=True, use_xyz=True))
...
encoder = Encoder(n=1024, embed=[3,32,'mlp','yes_ab'], # res_dim_ratio=0.25,
                  bias=False, use_xyz=True, norm_mode=["anchor","yes_ab"], std_mode="BN1D",
