...
DIM_RATIO='2-2-2-2'                # '2-2-2-1'

NUM_BLOCKS1='1-1-2-1'                # '1-1-2-1'
TRANSFER_MODE='no-no-no-no'         # 'mlp-mlp-mlp-mlp'
BLOCK1_MODE='cross_residual-cross_residual-cross_residual-cross_residual'           # 'mlp-mlp-mlp-mlp'
ADAPTIVE_RATIO_1=('0.9-0.9-0.9-0.9' '0.8-0.8-0.8-0.8' '1-1-1-1' '0.7-0.7-0.7-0.7')
ADAPTIVE_RES_DIM_RATIO_1='0.25-0.25-0.25-0.25'
AGMENTIVE_RES_DIM_RATIO_1='4-4-4-4'

NUM_BLOCKS2='1-1-2-1'        # '1-1-2-1'
BLOCK2_MODE='cross_residual-cross_residual-cross_residual-cross_residual'
...
for i in {1..100}; do
        for SEED in "${SEED[@]}"; do
                for ADAPTIVE_RATIO_1 in "${ADAPTIVE_RATIO_1[@]}"; do
                        python cls_modelnet.py --n "$N" --embed "$EMBED_Modelnet" --initial_dim "$INITIAL_DIM" --embed_dim "$EMBED_DIM" \
                        --norm_mode "$NORM_MODE" --std_mode "$STD_MODE" --sigma "$SIGMA" \
                        --dim_ratio "$DIM_RATIO" --num_blocks1 "$NUM_BLOCKS1" --block1_mode "$BLOCK1_MODE" \
                        ...
                        --num_blocks2 "$NUM_BLOCKS2" --block2_mode "$BLOCK2_MODE" \
                        --adaptive_ratio_2 "$ADAPTIVE_RATIO_1" \
                        --adaptive_res_dim_ratio_2 "$ADAPTIVE_RES_DIM_RATIO_2" \
                        --agmentive_res_dim_ratio_2 "$AGMENTIVE_RES_DIM_RATIO_2" \
                        --k_neighbors "$K_ModelNet" \
                        ...
                        --alpha_beta "$ALPHA_BETA" --seed "$SEED" --ema "$EMA"
